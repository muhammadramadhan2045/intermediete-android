package com.example.ticketingapps.custom_view
...
import com.example.ticketingapps.Seat
...
class SeatsView : View {
    ...
    private fun drawSeat(canvas: Canvas?, seat: Seat) {
        ...
        //mengatur warna ketika seat di klik/dibooking
        if (seat.isBooked) {
            backgroundPaint.color = ResourcesCompat.getColor(resources, R.color.grey_200, null)
            armestPaint.color = ResourcesCompat.getColor(resources, R.color.grey_200, null)
            ...
            armestPaint.color = ResourcesCompat.getColor(resources, R.color.blue_700, null)
        ...
        //Sandaran Tangan
        val armestPath = Path()
        armestPath.addRect(0F, 0F, 50F, 200F, Path.Direction.CCW)
        canvas?.drawPath(armestPath, armestPaint)
